# NES × Looking Glass — 立体視ファミコンエミュレーター

Looking Glass(ライトフィールドディスプレイ)向けのNES/ファミコンエミュレーターWebアプリです。
NES PPUのレイヤー構造を奥行き方向に分離し、ゲーム画面が立体的に浮き出て見えます。

- **ピクセル立体化** — 奥から「背景色 → 背面スプライト → 背景タイル → 前面スプライト」の
  順でZ方向に配置(NES PPUの合成順そのまま)。スプライトは近接するOAM要素を
  キャラクター単位に近いグループへまとめ、画面上の位置に応じて細かな奥行き差を付加。
  層間距離に加え、スプライトの結合距離・最大グループ数・個別奥行きをゲームごとに
  その場で調整可能
- **自由視点** — マウスでシーン全体を回転・拡大縮小・移動
- **Looking Glass対応** — [Looking Glass WebXR Library](https://github.com/Looking-Glass/looking-glass-webxr)
  によるquilt描画。Portrait / Go / 16" / 27" / 32" など(キャリブレーションはBridge経由で自動)
- **完全クライアントサイド** — ROMイメージはブラウザ内でのみ読み込まれ、
  サーバーには一切送信・保存されません
- **内蔵デモROM** — 動作確認用のデモROM(オリジナル、6502コードをブラウザ内で生成)を同梱

## 必要環境

| 項目 | 要件 |
|---|---|
| ブラウザ | Chromium系(Chrome / Edge)推奨。Safariは非対応 |
| Looking Glass表示 | [Looking Glass Bridge](https://lookingglassfactory.com/software/looking-glass-bridge) がインストール・起動済みであること |
| 通常表示 | Looking Glassがなくても普通のモニターで3D表示できます |

## 使い方

1. ページを開き、**「ROMを開く…」** で手元のiNES形式(.nes)ファイルを選択
   (ウィンドウへのドラッグ&ドロップも可)。まずは **「内蔵デモ」** でも動作確認できます
2. **「Looking Glassで表示」** を押すと表示用ウィンドウが開くので、
   Looking Glass側のディスプレイへ移動してダブルクリックで全画面化
3. **層間距離**、背景・スプライトの厚み、スプライトのまとめ方と奥行き差を調整
   「立体感」はシーンのZ倍率です。Looking Glassの出力ウィンドウは校正情報と画面配置を使って
   Looking Glass側へ配置されます。最大化ではなく、枠なしの全画面表示を使用してください。
   自動全画面が拒否された場合は、出力ウィンドウ内の全画面表示ボタンをクリックします。
   操作画面は2Dプレビューです。

校正情報を取得できない状態ではLooking Glass表示を開始しません。表示中はパネルに
校正解像度、出力ウィンドウの接続状態、実際のXR view数、前後サンプルの投影視差、
全画面表示と実ピクセルサイズの一致状態を表示します。
実view数が2未満、または視差が0の場合はBridgeと出力ウィンドウを確認してください。

### 操作

| 入力 | 動作 |
|---|---|
| ゲームパッド | 十字キー/左スティック=移動、右側ボタン=A、左側ボタン=B、Start/Select |
| キーボード | 矢印キー、`X`=A、`Z`=B、`Enter`=Start、`Shift`=Select |
| 左ドラッグ | シーン回転 |
| ホイール | 拡大縮小 |
| 右ドラッグ / Shift+ドラッグ | 移動 |
| ダブルクリック | 視点リセット |

### 対応マッパー

任天堂系(NROM / MMC1-5)に加え、国内メーカー系を広くカバーしています:

| 系統 | マッパー番号 |
|---|---|
| 任天堂 | 0, 1, 2, 3, 4, 5, 7, 9 (MMC2), 10 (MMC4) |
| Konami VRC | 21, 22, 23, 24, 25, 26 (VRC2/4/6), 73 (VRC3), 75 (VRC1), 85 (VRC7) |
| Namco | 19 (N163), 76, 88, 95, 154, 206 (Namco 108系), 210 (175/340) |
| Sunsoft | 67, 68, 69 (FME-7), 89, 93, 184 |
| Taito | 33, 48 (TC0190/TC0690), 80 (X1-005) |
| Irem | 32 (G-101), 65 (H3001), 78, 97 |
| Jaleco | 18 (SS88006), 72, 86, 87, 92 |
| Bandai | 16, 153, 159 (FCG/LZ93D50), 70, 152 |
| その他 | 11, 34, 38, 66, 71, 79, 94, 118, 119, 140, 180, 240, 241 |

既知の制限: VRC6/VRC7/N163/FME-7の拡張音源は未対応(ゲームは動作しますが
該当パートの音が鳴りません)。BandaiのEEPROMセーブはスタブ(起動可・保存不可)。
Sunsoft-4のROMネームテーブルは近似表示です。
Mapper 23(Konami VRC2b / VRC4e / VRC4f)とMapper 25
(Konami VRC2c / VRC4b / VRC4d)は、NES 2.0サブマッパーを含めて利用できます。

## 開発

```bash
npm install
npm run dev     # 開発サーバー
npm run build   # 型チェック + 本番ビルド (dist/)
```

`main` ブランチへのpushでGitHub Actionsがビルドし、GitHub Pagesへデプロイします
(リポジトリ設定でPagesのソースを「GitHub Actions」にしてください)。

## 仕組み

エミュレーションコアには [jsnes](https://github.com/bfirsh/jsnes) v2.1.0を
`src/emulator/vendor/jsnes/` に取り込み、PPUの合成処理に「背景タイル/
背面スプライト/前面スプライト」を別バッファへ複製出力する改造を加えています
(エミュレーション動作自体は無改変)。各レイヤーは毎フレーム
`THREE.DataTexture` としてthree.jsのプレーンに転送され、WebXR経由で
Looking Glassのquiltレンダリングに乗ります。

Looking Glassの視錐台は、画面の縦横比とBridgeから取得した校正値に合わせて自動調整します。
背景の表示面を焦点面(`world Z=0`)に置いたまま層間距離とZ倍率を適用するため、
立体感を変更しても焦点面が移動しません。実際のXR ArrayCameraの左右端viewから
投影視差を計測し、表示パネルへ診断値として反映します。

## 注意事項

- ROMイメージは**ご自身が権利を有するもの**のみお使いください
- 本プロジェクトは任天堂およびLooking Glass Factoryとは無関係の
  非公式ソフトウェアです

## ライセンス

本プロジェクトのコードは [MIT License](LICENSE) です。
取り込んでいるjsnes(Apache-2.0、改変あり)ほかサードパーティの表記は
[THIRD-PARTY-NOTICES.md](THIRD-PARTY-NOTICES.md) を参照してください。

## ピクセル立体化

ピクセル立体化は起動時から標準の表示方式です。透明ピクセルとの境界だけに側面を生成し、
NES画像を奥行き方向へ押し出すPortal NESの考え方をthree.js向けに独立実装し、
Looking Glassの多視点描画へ取り入れています。
背景とスプライトの厚み、層間距離、スプライトのグループ化を調整できます。
厚みは層の前後関係を保つため自動的に制限されます。Looking Glass表示は既存の
WebXRシーンをそのまま使用します。

## 謝辞

透明ピクセルとの境界に側面を作り、画像を奥行き方向へ押し出す方式を公開された
[vjroba氏](https://github.com/vjroba)の[Portal NES](https://github.com/vjroba/portal_nes)に
感謝します。本プロジェクトではその仕組みを参考に、three.js向けの独立実装として
Looking Glassの多視点描画に取り入れました。

本実装はPortal NESのコードや設定をコピーしたものではなく、完全な移植でも、
プロファイル互換でもありません。アルゴリズムの挙動を参考にした独立実装です。
