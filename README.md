...
- **レイヤー立体視** — 奥から「背景色 → 背面スプライト → 背景タイル → 前面スプライト」の
  4層をZ方向に配置(NES PPUの合成順そのまま)。層間距離はスライダーで調整可能
- **AI深度モード(実験的)** — 単眼深度推定モデル
  [Depth Anything V2 (small)](https://huggingface.co/onnx-community/depth-anything-v2-small)
  をWebGPUでリアルタイム実行し、ゲーム画面から推定した深度マップで
  レリーフ状に立体化。「深度強さ」スライダーで調整可能。
...
3. **立体化** セレクタでモードを選択(レイヤー分離 / AI深度)し、
