...
- **ピクセル立体化** — 奥から「背景色 → 背面スプライト → 背景タイル → 前面スプライト」の
...
3. **層間距離**、背景・スプライトの厚み、スプライトのまとめ方と奥行き差を調整
...
ピクセル立体化は起動時から標準の表示方式です。透明ピクセルとの境界だけに側面を生成し、
NES画像を奥行き方向へ押し出すPortal NESの考え方をthree.js向けに独立実装し、
Looking Glassの多視点描画へ取り入れています。
背景とスプライトの厚み、層間距離、スプライトのグループ化を調整できます。
厚みは層の前後関係を保つため自動的に制限されます。Looking Glass表示は既存の
WebXRシーンをそのまま使用します。

## 謝辞

透明ピクセルとの境界に側面を作り、画像を奥行き方向へ押し出す方式を公開された
[vjroba氏](https://github.com/vjroba)の[Portal NES](https://github.com/vjroba/portal_nes)に
感謝します。本プロジェクトではその仕組みを参考に、three.js向けの独立実装として
Looking Glassの多視点描画に取り入れました。

本実装はPortal NESのコードや設定をコピーしたものではなく、完全な移植でも、
プロファイル互換でもありません。アルゴリズムの挙動を参考にした独立実装です。
