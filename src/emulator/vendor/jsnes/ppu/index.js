 ...
 * Added per-layer output buffers (sprFrontBuffer / sprBehindBuffer /
 * layerBackdropColor) so that background and sprites can be rendered as
 * separate depth layers on a Looking Glass display. Emulation behaviour is
 * unchanged; the layer buffers are write-only side outputs.
 * Original: jsnes v2.1.0 <https://github.com/bfirsh/jsnes>, Apache-2.0.
 * See LICENSE and README.md in the vendor directory.
 */
...
// [nesemu-lookingglass modification] Sentinel for "this layer did not draw
// this pixel". Palette colors are 24-bit, so 0xffffffff is unambiguous.
export const LAYER_NONE = 0xffffffff;
...
class PPU {
  ...
  constructor(nes) {
    ...
    // [nesemu-lookingglass modification] Per-layer side outputs for
  ...
  startFrame() {
    ...
    // [nesemu-lookingglass modification] Reset layer outputs and remember the
    // backdrop color for the deepest plane.
